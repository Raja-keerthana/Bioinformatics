...
def _hierarchical_reduce(
    ...
    if len(combined) <= char_budget:
        ...
        return generate_answer(prompt, llm_pipeline, max_new_tokens=max_new_tokens)
    ...
    batch_summaries = [
        generate_answer(build_prompt_fn("\n".join(batch)), llm_pipeline, max_new_tokens=max_new_tokens)
...
def generate_literature_review(chunk_metadata: list, llm_pipeline: dict) -> dict:
    ...
    for section_title, section_instruction in LITERATURE_REVIEW_SECTIONS:
        ...
        review[section_title] = generate_answer(prompt, llm_pipeline, max_new_tokens=150)
